...
sub getEncKey {
	my $o = tie my @array, 'Tie::File', $VAR{enc_key} or die "I can't open miquiloni encrypted key: $VAR{enc_key}\n";
	my $encKey = $array[0];
	untie @array;
	return $encKey;
}
...
sub get_permissions {
	...
	$PERM{settings} = $perm[4];
	$PERM{docs} = $perm[5];
	$PERM{lxcservers} = $perm[6];
	$PERM{lxcservers_edit} = $perm[7];
	$PERM{provisioning} = $perm[8];
	$PERM{accounts} = $perm[9];
	$PERM{accounts_edit} = $perm[10];
	$PERM{containers} = $perm[11];
...
sub login {
	...
	<html xmlns="http://www.w3.org/1999/xhtml">
		<head>
			...
			<title>LogIn :: MIQUILONI :: LXC Management Tool</title>
			<meta name="keywords" content="lxc, containers, snapshot, recovery, linux" />
